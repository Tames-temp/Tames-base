...
public class Utils
{
    ...
    public static float VerticalRotation(Quaternion a, Quaternion b)
    {
        Vector3 af = a * Vector3.forward;
        Vector3 bf = b * Vector3.forward;
        if ((bf.y != 1) && (af.y != 1))
        {
            Debug.Log("vr1: " + af.ToString("0.000") + af.ToString("0.000"));
            return Angle(bf, Vector3.zero, af, Vector3.up, true);
        }
        else
        {
            af = a * Vector3.right;
            bf = b * Vector3.right;
            Debug.Log("vr2: " + af.ToString("0.000") + af.ToString("0.000"));
            if ((bf.y != 1) && (af.y != 1))
                return Angle(bf, Vector3.zero, af, Vector3.up, true);
            else
            {
                af = a * (Vector3.up + Vector3.right);
                bf = b * (Vector3.up + Vector3.right);
                Debug.Log("vr3: " + af.ToString("0.000") + af.ToString("0.000"));
                return Angle(bf, Vector3.zero, af, Vector3.up, true);
            }
        }
    }
